...
### Requirement: Capture expression parsing (from dsl-parsing)

**PREVIOUS**: Within a DSL capture (`#{...}`), the parser SHALL parse a single AgentScript expression using the standard expression parser. Statements SHALL NOT be allowed — only expressions.

**NEW**: Within a DSL capture (`#{...}`), the parser SHALL parse the content as a block body: statements followed by an optional tail expression. If the content is a single expression, it SHALL be used directly (backward compatible). If statements are present, the content SHALL be wrapped in `Expr::Block`.
